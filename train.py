...
setting = Setting()
setting.parse()
log = setting.log_file + 'gpu{}_lr{}_dim{}_result'.format(setting.gpu, setting.learning_rate, setting.hidden_dim)
log = open(log, 'w')
...
# load dataset
if not os.path.exists('poi_loader.pkl'):
    poi_loader = PoiDataloader(
        setting.max_users, setting.min_checkins)  # 0， 5*20+1
    poi_loader.read(setting.dataset_file)
    pickle.dump(poi_loader, open('poi_loader.pkl', 'wb'))
else:
    poi_loader = pickle.load(open('poi_loader.pkl', 'rb'))
...
# Split niche and hot items

l = [item[1] for item in poi_loader.poi_freq.items()]
l = sorted(l)
niche_threshold = l[int(len(l)*setting.niche_threshold)]
log_string(log, 'Hot item threshold: {}'.format(niche_threshold))

niche = set()
hot = set()
for poi in poi_loader.poi_freq:
    if poi_loader.poi_freq[poi] <= niche_threshold:
        niche.add(poi)
    else:
        hot.add(poi)
log_string(log, 'Niche poi: {}'.format(len(niche)))
log_string(log, 'Hot poi: {}'.format(len(hot)))
...
with open(setting.trans_interact_file, 'rb') as f:  # User-POI的图，根据visiting relation构建，估计就是user访问过哪些poi
...
trainer.prepare(poi_loader.locations(), poi_loader.user_count(), setting.hidden_dim, setting.rnn_factory,
                setting.device, poi_loader.pindex2grid, poi_loader.user_region_graph)
...
for e in range(setting.epochs):  # 100
    ...
    for i, (x, t, t_slot, s, r, y, y_t, y_t_slot, y_s, y_r, reset_h, active_users) in enumerate(dataloader):
        ...
        y_r = y_r.squeeze().to(setting.device)
        active_users = active_users.to(setting.device)

        optimizer.zero_grad()
        forward_start = time.time()
        loss = trainer.loss(x, t, t_slot, s, r, y, y_t,
                            y_t_slot, y_s, y_r, h, active_users)
        ...
        # nn.utils.clip_grad_norm_(trainer.model.parameters(), max_norm=1, norm_type=2)
        nn.utils.clip_grad_value_(trainer.model.parameters(), clip_value=0.1)
        optimizer.step()
    ...
    if (e + 1) % setting.validate_epoch == 0:
        ...
        evl_start = time.time()
        evaluation_test.evaluate(niche, hot)
        evl_end = time.time()
