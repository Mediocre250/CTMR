...
class FlashbackTrainer():
    ...
    def prepare(self, loc_count, user_count, hidden_size, gru_factory, device, loc2grid, user_region_graph):
        ...
        self.model = Flashback(loc_count, user_count, hidden_size, f_t, f_s, gru_factory, self.lambda_loc,
                               ...
                               self.use_graph_user, self.use_spatial_graph, self.interact_graph, loc2grid, user_region_graph, device).to(device)

    def evaluate(self, x, t, t_slot, s, r, y_t, y_t_slot, y_s, y_r, h, active_users):
        ...
        out, h, region_pred = self.model(x, t, t_slot, s, r, y_t,
    ...
    def loss(self, x, t, t_slot, s, r, y, y_t, y_t_slot, y_s, y_r, h, active_users):
        """ takes a batch (users x location sequence)
        and corresponding targets in order to compute the training loss """

        self.model.train()
        out, h, region_pred = self.model(x, t, t_slot, s, r, y_t, y_t_slot, y_s, y_r, h,
                            active_users)  # out (seq_len, batch_size, loc_count)
        # out, h = self.model(x, t, t_slot, s, r, y_t, y_t_slot, y_s, y_r, h,
        #             active_users)  # out (seq_len, batch_size, loc_count)
        out = out.view(-1, self.loc_count)  # (seq_len * batch_size, loc_count)

        region_pred = region_pred.view(-1, 264)

        y = y.view(-1)  # (seq_len * batch_size)
        y_r = y_r.view(-1)
        l = self.cross_entropy_loss(out, y) + 2 * self.cross_entropy_loss(region_pred, y_r)
